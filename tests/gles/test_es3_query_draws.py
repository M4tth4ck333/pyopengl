...
from OpenGL.GLES3 import (
    GL_ARRAY_BUFFER, GL_ELEMENT_ARRAY_BUFFER, GL_COPY_WRITE_BUFFER,
    GL_STATIC_DRAW, GL_BUFFER_SIZE, GL_BUFFER_MAP_POINTER,
    GL_MAP_WRITE_BIT, GL_MAP_FLUSH_EXPLICIT_BIT,
    GL_FLOAT, GL_FALSE, GL_TRIANGLES, GL_UNSIGNED_INT,
    GL_ANY_SAMPLES_PASSED, GL_CURRENT_QUERY, GL_QUERY_RESULT,
    glUseProgram, glGetAttribLocation,
    glGenVertexArrays, glBindVertexArray,
    glGenBuffers, glBindBuffer, glBufferData,
    glEnableVertexAttribArray, glVertexAttribPointer, glVertexAttribDivisor,
    glDrawArraysInstanced, glDrawElementsInstanced, glDrawRangeElements,
    glGenQueries, glBeginQuery, glEndQuery, glGetQueryiv, glGetQueryObjectuiv,
    glIsQuery, glDeleteQueries,
    glCopyBufferSubData, glGetBufferParameteri64v, glGetBufferPointerv,
...
class TestES3QueryDraws(ESTestCase):
    ...
    def setUp(self):
        ...
        glBufferData(
            GL_ARRAY_BUFFER, 24,
            np.array([(-1, -1), (1, -1), (0, 1)], 'f'), GL_STATIC_DRAW,
        )
        ...
        glBufferData(
            GL_ELEMENT_ARRAY_BUFFER, 12, np.array([0, 1, 2], np.uint32), GL_STATIC_DRAW,
        )
    ...
    def test_buffer_copy_and_query(self):
        ...
        glGetBufferPointerv(GL_ARRAY_BUFFER, GL_BUFFER_MAP_POINTER, ctypes.byref(pointer))
