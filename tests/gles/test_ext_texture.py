...
from OpenGL.GLES3 import (
    GL_TEXTURE_2D, GL_TEXTURE_3D, GL_RGBA, GL_RGBA8, GL_R32UI, GL_UNSIGNED_BYTE,
...
from OpenGL.GLES2.OES.texture_storage_multisample_2d_array import GL_TEXTURE_2D_MULTISAMPLE_ARRAY_OES
...
class TestTextureExtensions(ESTestCase):
    ...
    def test_ext_texture_border_clamp(self):
        ...
        with self.exercise():
            self._tex()
            ext_border.glTexParameterIivEXT(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR_EXT, BORDER_I)
            ext_border.glTexParameterIuivEXT(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR_EXT, BORDER_U)
            ext_border.glGetTexParameterIivEXT(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR_EXT, np.zeros(4, 'i'))
            ext_border.glGetTexParameterIuivEXT(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR_EXT, np.zeros(4, 'u4'))
            from OpenGL.GLES3 import glGenSamplers
            s = glGenSamplers(1)
            ext_border.glSamplerParameterIivEXT(s, GL_TEXTURE_BORDER_COLOR_EXT, BORDER_I)
            ext_border.glSamplerParameterIuivEXT(s, GL_TEXTURE_BORDER_COLOR_EXT, BORDER_U)
            ext_border.glGetSamplerParameterIivEXT(s, GL_TEXTURE_BORDER_COLOR_EXT, np.zeros(4, 'i'))
            ext_border.glGetSamplerParameterIuivEXT(s, GL_TEXTURE_BORDER_COLOR_EXT, np.zeros(4, 'u4'))
    ...
    def test_oes_texture_border_clamp(self):
        ...
        with self.exercise():
            self._tex()
            oes_border.glTexParameterIivOES(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR_OES, BORDER_I)
            oes_border.glTexParameterIuivOES(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR_OES, BORDER_U)
            oes_border.glGetTexParameterIivOES(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR_OES, np.zeros(4, 'i'))
            oes_border.glGetTexParameterIuivOES(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR_OES, np.zeros(4, 'u4'))
            from OpenGL.GLES3 import glGenSamplers
            s = glGenSamplers(1)
            oes_border.glSamplerParameterIivOES(s, GL_TEXTURE_BORDER_COLOR_OES, BORDER_I)
            oes_border.glSamplerParameterIuivOES(s, GL_TEXTURE_BORDER_COLOR_OES, BORDER_U)
            oes_border.glGetSamplerParameterIivOES(s, GL_TEXTURE_BORDER_COLOR_OES, np.zeros(4, 'i'))
            oes_border.glGetSamplerParameterIuivOES(s, GL_TEXTURE_BORDER_COLOR_OES, np.zeros(4, 'u4'))
    ...
    def test_oes_texture_3d(self):
        ...
        with self.exercise():
            ...
            oes_3d.glTexImage3DOES(GL_TEXTURE_3D, 0, GL_RGBA, 2, 2, 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, vol)
            oes_3d.glTexSubImage3DOES(GL_TEXTURE_3D, 0, 0, 0, 0, 2, 2, 2, GL_RGBA, GL_UNSIGNED_BYTE, vol)
            oes_3d.glCopyTexSubImage3DOES(GL_TEXTURE_3D, 0, 0, 0, 0, 0, 0, 2, 2)
            oes_3d.glCompressedTexImage3DOES(GL_TEXTURE_3D, 0, GL_COMPRESSED_RGB8_ETC2, 4, 4, 1, 0, 8, np.zeros(8, 'u1'))
            oes_3d.glCompressedTexSubImage3DOES(GL_TEXTURE_3D, 0, 0, 0, 0, 4, 4, 1, GL_COMPRESSED_RGB8_ETC2, 8, np.zeros(8, 'u1'))
            fbo = glGenFramebuffers(1)
            glBindFramebuffer(GL_FRAMEBUFFER, fbo)
            oes_3d.glFramebufferTexture3DOES(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_3D, self._tex(GL_TEXTURE_3D), 0, 0)
    ...
    def test_ext_clear_texture(self):
        ...
        with self.exercise():
            ...
            ext_clear.glClearTexImageEXT(tex, 0, GL_RGBA, GL_UNSIGNED_BYTE, np.zeros(4, 'u1'))
            ext_clear.glClearTexSubImageEXT(tex, 0, 0, 0, 0, 2, 2, 1, GL_RGBA, GL_UNSIGNED_BYTE, np.zeros(4, 'u1'))
    ...
    def test_ext_texture_storage(self):
        ...
        with self.exercise():
            ...
            tex = self._tex()
            ext_storage.glTexStorage1DEXT  # 1D unavailable in ES; referenced only
    ...
    def test_copy_image(self):
        ...
        with self.exercise():
            ...
            b = self._tex()
            glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA8, 8, 8)
            ext_copy.glCopyImageSubDataEXT(a, GL_TEXTURE_2D, 0, 0, 0, 0, b, GL_TEXTURE_2D, 0, 0, 0, 0, 8, 8, 1)
    ...
    def test_oes_copy_image(self):
        ...
        with self.exercise():
            ...
            b = self._tex()
            glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA8, 8, 8)
            oes_copy.glCopyImageSubDataOES(a, GL_TEXTURE_2D, 0, 0, 0, 0, b, GL_TEXTURE_2D, 0, 0, 0, 0, 8, 8, 1)
    ...
    def test_oes_msaa_array_storage(self):
        ...
        with self.exercise():
            ...
            oes_msaa.glTexStorage3DMultisampleOES(GL_TEXTURE_2D_MULTISAMPLE_ARRAY_OES, 4, GL_RGBA8, 8, 8, 2, True)
