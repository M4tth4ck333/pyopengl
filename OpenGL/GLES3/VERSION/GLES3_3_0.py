...
# Accept a Python list of names (deriving count) for the char** commands, as the
# glShaderSource wrapper in GLES2_2_0.py does.
from OpenGL.lazywrapper import lazy as _lazy
from OpenGL._bytes import as_8_bit as _as_8_bit


def _string_array(names):
    if isinstance(names, (bytes, str)):
        names = [names]
    names = [_as_8_bit(n) for n in names]
    array = (ctypes.c_char_p * len(names))(*names)
    return len(names), ctypes.cast(
        array, ctypes.POINTER(ctypes.POINTER(ctypes.c_char))
    )


@_lazy(glTransformFeedbackVaryings)
def glTransformFeedbackVaryings(baseOperation, program, varyings, bufferMode):
    """glTransformFeedbackVaryings(program, ['name', ...], bufferMode)"""
    count, array = _string_array(varyings)
    return baseOperation(program, count, array, bufferMode)


@_lazy(glGetUniformIndices)
def glGetUniformIndices(baseOperation, program, uniformNames, uniformIndices=None):
    """glGetUniformIndices(program, ['name', ...]) -> uniformIndices array"""
    count, array = _string_array(uniformNames)
    if uniformIndices is None:
        uniformIndices = arrays.GLuintArray.zeros((count,))
    baseOperation(program, count, array, uniformIndices)
    return uniformIndices



# friendly glDelete*(array) forms (see GLES2_2_0._delete_wrapper)
from OpenGL.GLES2.VERSION.GLES2_2_0 import _delete_wrapper as _delete_wrapper
glDeleteVertexArrays = _delete_wrapper(glDeleteVertexArrays)
glDeleteQueries = _delete_wrapper(glDeleteQueries)
glDeleteSamplers = _delete_wrapper(glDeleteSamplers)
glDeleteTransformFeedbacks = _delete_wrapper(glDeleteTransformFeedbacks)
